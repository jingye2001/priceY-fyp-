...
<div class="col-md-12">   
    @if(isset($laptops) && count($laptops) > 0)
        <div class="row">
            <div class="col-md-12">
                <h3 class="manu_title">{{ $manufacturer }}</h3>
            </div>
        </div>
        <div class="row">
                ...
                <div class="col-md-3 col-6">
                    <div class="product-grid">
                        <div class="product-image">
                            <a href="{{ route('laptopDetails', ['id' => $laptop->id]) }}" class="image">
                                <img src="{{ asset('images/') }}/{{$laptop->manufacturer}}/{{$laptop->image}}" onclick="event.preventDefault();">
                            </a>
                            
                            <ul class="product-links">
                                <li>   
                                    <form method="post" action="{{ route('favorites.toggle', ['laptopId' => $laptop->id]) }}">
                                        @csrf
                                        <button type="submit" class="favorite-button">
                                            <i class="fa {{ $laptop->isFavoritedByUser(auth()->user()->id) ? 'fa-heart red-heart' : 'fa-heart gray-heart' }}"></i>
                                        </button>
                                    </form>
                                </li>
                                <li><a href="{{ route('compareLaptop', ['id' => $laptop->id]) }}"><img src="{{ asset('images/compare logo.jpg') }}"></a></li>
                            </ul>
                            <a href="{{ route('laptopDetails', ['id' => $laptop->id]) }}" class="add-to-cart">View laptop</a>
                        </div>
    ...
    <div class="noinformation">
        <h5 class="no-text">No manufacturers found.</h5>
    </div>
    @endif
    </div>
</div>
